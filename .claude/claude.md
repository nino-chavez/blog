...
- **Voice guide enforcement:** All blog content follows `/docs/signal-dispatch-voice-guide.md` (v1.5). Open with the defect in hand (concrete broken/missing thing + scale), show the work (re-derive claims from the primary source, report counts), self-interrogate procedurally mid-post, close on a compressed reversal — final sentence under 12 words, concrete. No stock provisional closes ("here's where I've landed—for now" is a retired tell). For any content derived from another piece (captions, excerpts, syndication copy), apply guide §1b — scope preservation: the derived text keeps the subject the source assigned it, **neither narrower nor wider**. Narrower is relocation (source says "the check people skip", draft says "the step I kept deferring"). Wider is invented generalization (source says a README of his drifted, draft says "a README is the file you write once and never open again"). Both invent nothing and both pass the "which friend?" test, which is why they ship. Verify with `node syndication/check-captions.mjs`.
