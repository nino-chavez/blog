...
- **Voice guide enforcement:** All blog content follows `/docs/signal-dispatch-voice-guide.md` (v1.7). Apply its stack in order: evidence and claim status → reader and artifact job → controlling argument → cognitive load → Signal Dispatch voice → surface mechanics. State what the piece establishes within 150 words. Show primary evidence when the claim depends on it, not to manufacture an investigation scene. Do not turn session steering, revision churn, or a model's objection into POV movement unless new evidence changed the claim. Corpus traits such as a defect-in-hand opener, question count, sentence distribution, self-correction, adverse-evidence section, and compressed close are diagnostics, not gates. An open ending is valid only after the answer lands. For any content derived from another piece (captions, excerpts, syndication copy), apply guide §1b — scope preservation: the derived text keeps the subject the source assigned it, **neither narrower nor wider**. Narrower is relocation (source says "the check people skip", draft says "the step I kept deferring"). Wider is invented generalization (source says a README of his drifted, draft says "a README is the file you write once and never open again"). Both invent nothing and both pass the "which friend?" test, which is why they ship. Verify with `node syndication/check-captions.mjs`.
