A safety check can be mechanically right and still tell a person something false.

That is the failure this piece turns on, and writing it down never occurred to me. The tool in question lets a club administrator change website copy by describing what they want in plain words. Everything genuinely risky about that, I planned for in detail: the suggestion comes from a language model, but the decision to write is made by code that never talks to one, and each edit is spliced in at known offsets behind a content hash.

Then I refreshed the page mid-apply. The resubmit was caught and rejected, because the page no longer matched the proposal. It no longer matched because the edit had already gone through. So the log marked the edit rejected while that same edit sat finished on the screen.
