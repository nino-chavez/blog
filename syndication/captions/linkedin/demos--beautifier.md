...
A tool named for decoration doing its most valuable work as a diagnostic isn't magic, and the reason is unglamorous. Its instructions told it to read the real repository first — check the claim against the thing it describes, the config for a deploy line, the lockfile for a package manager, the dependency list for a feature list.

The failure it produced on me is almost too on the nose. I wrote that the hand-drawn banners would render fine against a dark background, having only ever looked at them locally in a preview app and never once on the page where they actually ship. I asserted the render and skipped the render.
