...
So I added three layers. Each has one job. An AGENTS.md that orients, then asks the human two questions before anything runs. A CLAUDE.md that constrains rather than teaches. And a command that emits a structured payload generated from the tool's own schema.
...
Rules only work when violation is detectable. "Exporters are pure" survives because you can grep the directory for a network call. "Write clean code" does nothing at all.
