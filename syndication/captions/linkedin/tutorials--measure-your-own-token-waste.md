...
The thing that reframed it is boring and structural. The agent API is stateless. Every single tool call resends the entire conversation from the top. Nothing is free just because you already said it once. A token that lands early in a long session gets paid for again on every turn that follows. That makes session cost roughly quadratic in turn count. Across 529 sessions it came out as a replay multiplier of 41 — cache reads over tokens written. So the real driver is session length, not any individual action that feels expensive. I'd been trimming what I sent. Precisely the wrong lever.
