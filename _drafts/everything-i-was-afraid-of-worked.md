...
The fan-out pattern has a name and a published shape, and the published shape includes a step where the results get reconciled before anything is used. I built the fan-out and skipped the reconciliation, without ever knowing I was implementing a named pattern. The fix is to let the model see all the matched text at once and decide the wording a single time. It also happens to collapse hundreds of calls into a handful, which is how I first mistook a correctness bug for a speed bug.
