    ...
    >
      <div className="relative">
        ...
        <button
          ...
          className={`px-3 sm:px-4 py-2 sm:py-3 min-h-[40px] sm:min-h-[44px] rounded-full text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-150 flex-shrink-0 ${
        ...
        {primaryCategories.map((category) => {
          ...
          return (
            <button
              ...
              className={`px-3 sm:px-4 py-2 sm:py-3 min-h-[40px] sm:min-h-[44px] rounded-full text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-150 border flex-shrink-0 ${buttonClasses} ${isSelected ? 'shadow-lg scale-105' : 'hover:scale-105'}`}
            >
              {category}
        ...
        {overflowCategories.length > 0 && (
          <div className="relative flex-shrink-0" ref={dropdownRef}>
            <button
              ...
              className={`px-3 sm:px-4 py-2 sm:py-3 min-h-[40px] sm:min-h-[44px] rounded-full text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-150 flex items-center gap-1.5 sm:gap-2 border ${getCategoryButtonClass(selectedCategory || '', isOverflowCategorySelected)} ${isOverflowCategorySelected ? 'shadow-lg scale-105' : 'hover:scale-105'}`}
            >
                ...
                {overflowCategories.map((category) => {
                  ...
                  return (
                    <button
                      key={category}
                      onClick={() => handleCategoryClick(category)}
                      className={`w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm font-medium transition-colors duration-150 flex items-center justify-between ${buttonClasses}`}
                    >
                      <span>{category}</span>
