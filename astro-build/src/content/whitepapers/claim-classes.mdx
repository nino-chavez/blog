...
| Misses | A wrong figure with an honest, wrong formula beside it | Figures deliberately phrased to avoid the contract |
...
**The rule:** a shipping deliverable carrying derived figures — a workable trigger is two or more percentage figures — must declare somewhere how they were produced, or it is blocked. An individual figure with nothing attributing it raises a warning.

**The design detail that decides whether it works:** the predicate must not be lexical. A document that merely contains the word "methodology" in ambient prose has not declared anything, and a naive keyword check passes it. The declaration has to be structurally deliberate — a derivation note adjacent to the figures, or an actual methodology section — or the gate degrades into a word search and starts certifying documents it never examined.

**What tier 1 buys:** it converts the failure from *silent* to *stated*. A number with a declared derivation can be argued with by a reader who knows the domain. A number with nothing attached is indistinguishable from a number that was checked. That distinction is most of the practical value, and it costs a lint.

**What tier 1 does not buy:** correctness. It is a completeness check on the *paperwork*, not a verification of the *value*. Reporting it as verification reproduces the original error one level up.
