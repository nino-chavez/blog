---
title: "I Configured My Agent to Agree With Me"
publishedAt: "2026-07-20T18:30:00.000Z"
author: "Nino Chavez"
excerpt: "A rival model's review of my agent's work looked sharper — it pushed back, it demanded alternatives. So I ran a controlled experiment on my own harness: same model, same prompt, config on and off. The vanilla arm argued with me. Mine never did."
category: "AI & Automation"
featureImage: "/images/generated/i-configured-my-agent-to-agree-with-me.webp"
...
The design is small enough to describe in a paragraph. Take the pivotal message from the original session — the moment I said the design system *"needs to be thrown out and redefined from first principles"* — and freeze it as a fixed prompt. Run it through the same model under four harnesses:

- My full daily configuration
- A bare mode with every preference file, hook, and memory stripped off
- The full config plus one candidate rule
- Bare mode plus a single suspect config block in isolation
...
The first finding let my config off the hook. The bare model — no instructions from me at all — also produced a single committed design every time. The habit the reviewer exploited is the model's own default. My configuration didn't create it; it just never counteracted it.

And one appended sentence *did* counteract it, both times it ran: the arm carrying a rule that says developing two or three candidates on architecture decisions is the work, not hesitation, produced a genuine three-way comparison and cited the rule while doing it.
...
This failure mode is invisible by construction. Sycophancy in a chat interface leaves fingerprints — the flattery, the "great question." Suppressed pushback leaves nothing. You cannot see the objection your agent didn't raise.
</Callout>

The sycophancy discourse mostly blames the models, and the models have earned some of it. But a harness can install the same disease one reasonable rule at a time. I ran that configuration for months and the transcripts looked like productivity.
...
The configuration is still there. Most of it earns its place; the grid said so. But I notice I'm reading agent transcripts differently now — not for what's in them, but for what isn't.

<PullQuote>
An agent that never pushes back isn't agreeing with you. It might just be complying.
</PullQuote>
