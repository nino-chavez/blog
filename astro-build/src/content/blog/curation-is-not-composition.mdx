...
I've been sitting with [the post I wrote earlier this week](/blog/the-walk-in-is-full) — the one arguing that cost was a reflex, not a virtue — and an image keeps surfacing that I can't shake.

It's a specific look that defined the first years of desktop publishing. If you're old enough, you can picture it without me describing it. Five fonts on the cover. Headlines in Impact because Impact had just arrived and the person laying out the page had never seen it before. Straight margins wobbling because somebody had discovered justified text without discovering what it does to line spacing. The italicized *joyful*, twice per paragraph. Church bulletins and community newsletters and small-town real-estate flyers, all hit by the same flood at the same time, all looking a little bit like ransom notes.

These things read as jokes when I first saw them. I couldn't have told you at the time why I thought they were funny, except that I'd grown up around professionally-typeset books and the new desktop-published stuff looked like someone's cousin had hit the keyboard. What I was actually looking at — this only hit me forty years later — was the first minutes of a species-level experiment. *What happens when you drop the cost of making something, and the consensus that used to be enforced by a profession suddenly has no forcing function, and nobody notices because everyone is too excited to make their own thing?*
...
A bad painting doesn't make future paintings worse. A bad song doesn't contaminate the song on the shelf next to it. A bad essay sits in a folder on Substack and fails to be read, and the failure is contained. All of these are **static outputs**. You can ignore the bad ones.

Software is not static. Software is a system. Every piece of software exists inside a dependency graph — it imports other software, is imported by other software, touches data, triggers workflows, runs in production next to other software that doesn't know it's broken. Bad software doesn't sit on a shelf being bad. It *propagates*. Every other piece of software that calls it inherits its problems. Jevons plus static outputs produces a flood you can mostly ignore. Jevons plus compounding produces pollution in the water supply.
...
Art needs a consumer to matter. You have to listen to a song for it to affect you. You have to watch a film. You have to read a book. Bad art in isolation does nothing — it waits, inert, for an audience. The audience is a gate.

Software has no gate. Software runs. It touches data, takes money, moves records, authenticates users, syncs state, calls APIs, fires webhooks. It does things whether or not anyone is paying attention. A bad song needs to be heard to be bad. A bad library can silently corrupt a database for six months before anyone notices. The consequences are not mediated by engagement. They are mediated by *the software executing*, which is happening all the time, everywhere, in the background, with no one watching.

Every bad-software story I can think of is a story about something failing that nobody knew was there. The badness wasn't on the shelf. It was in the plumbing.
...
An album doesn't need to interoperate with the album next to it. You don't need Radiohead to be structurally compatible with Led Zeppelin for both to live on the same playlist. A book doesn't need to share a data format with the book on the shelf beside it. Art doesn't compose. Art *accumulates*, and accumulation is not the same thing.
...
That move doesn't work for software. And here's the reason: **the problem software has isn't discovery. It's composition.**

<PullQuote>Curation rescues art by finding the good pieces. Software doesn't have "pieces" — it has systems. A system of individually-curated bad components is still a broken system.</PullQuote>

A recommendation engine can surface a good library. It cannot make the good library *work with the other libraries you're using*. The composition problem is upstream of the discovery problem, and it doesn't get solved by finding better individual components. It gets solved by the components having been built against a shared set of assumptions — which is the exact thing the atrophying consensus reflex used to produce, and which is exactly what agent-generated software does not have.
...
The church bulletin in Comic Sans was funny. The software equivalent of the church bulletin in Comic Sans is the database schema that silently corrupts your records for six months. I wish I thought someone was going to Spotify it away.
